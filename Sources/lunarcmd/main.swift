...
do {
    ...
    if argv.contains("--no-fps-booster") {
        for i in try FileManager.default.contentsOfDirectory(atPath: homeDir + "/.lunarcmd_data/offline/\(versionLaunching)") {
            if i.contains(".jar") && !(i.lowercased().contains("sodium") || i.lowercased().contains("iris") || i.lowercased().contains("indium") || i.lowercased().contains("optifine")) {
                repeatIndex+=1
#if DEBUG
